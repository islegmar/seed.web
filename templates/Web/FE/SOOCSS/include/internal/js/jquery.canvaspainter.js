...
;(function($) {
  ...
  function Plugin(element, options) {
    ...
    function init() {
      ...
      if ( options.colors ) {
        ...
        for (var ind=0; ind<options.colors.length; ++ind) {
          ...
          $colorSelector.bind('click', {color : color}, function(evt){
            ...
            if ( options.isDebug ) console.log('color ' + color  + ' selected');
      ...
      if ( options.grosores) {
        ...
        for (var ind=0; ind<options.grosores.length; ++ind) {
          ...
          $grosorSelector.bind('click', {grosor : grosor}, function(evt){
            ...
            if ( options.isDebug ) console.log('grosor ' + grosor + ' selected');
            // Mark this color as selected
      ...
      $pLienzo.bind('_startPaint', function(evt, punto) {
        /*context.beginPath();
        context.arc(punto[0], punto[1],  Math.floor(Math.random() * (70 - 10 + 1)) + 10, 0, 2 * Math.PI, false);
        context.lineWidth = 5;
        ...
        context.lineTo(punto[0]+100, punto[1]);
        context.stroke();

        context.lineTo(punto[0]+100, punto[1]+100);
        context.stroke();
      ...
      $pLienzo.bind('startPaint', function(evt, punto) {
        if ( options.isDebug ) console.log('Start Paint in (' + punto[0] + ',' + punto[1] + ')');
        isPainting = true;
        lastPoint  = punto;
        ...
        // Begin the path
        context.beginPath();
        context.moveTo(punto[0], punto[1]);
      });
      $pLienzo.bind('painting', function(evt, punto) {
        if ( isPainting ) {
          if ( options.isDebug ) console.log('Painting to (' + punto[0] + ',' + punto[1] + ')');
          context.lineTo(punto[0], punto[1]);
          context.stroke();
        } 
      });
      $pLienzo.bind('endPaint', function(evt, punto) {
        if ( options.isDebug ) console.log('End Paint');
        isPainting = false;
        lastPoint  = punto;
      });
    ...
    function putImage(image) {
      ...
      if ( options.isDebug ) console.log('putImage> w : ' + w + ', h : ' + h + ', src : ' + image.src + '.');
      
      // Clean the canvas (setting the width/height values)
      canvas.width = w;
      canvas.height = h;
      
      // Draw this image in the canvas
